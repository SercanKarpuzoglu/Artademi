...
> **Son güncelleme:** 2026-06 / **TEACHER /mine + Dashboard + Logo + Keycloak login teması tamamlandı.** Daha önce: subscription (V14), platform fazı, backend çekirdeği (V1–V14), tüm web modülleri.
...
Migration sırası V1→V14 (V13=tenant, V14=subscription). **Toplam 184 test yeşil** (user + platform + subscription dahil). Tüm yazma uçları rol-korumalı.
...
### 7.3 Öğretmen — `teacher` (V5)
- Alanlar: ad/soyad/telefon/email, **keycloakUserId** (Keycloak sub eşleşmesi), hakedisTipi (`SAATLIK`/`CIRO_ORANI`). `TeacherBranch` açık entity.
- **Uçlar:** `/api/teachers` (POST/GET/PUT, `?aktif=&q=&bransId=`, `/{id}/active`). Yetki: yazma ADMIN; okuma 3 rol.

### 7.4 Grup — `group` (V6, `@Table(name="lesson_group")`)
...
### 7.5 Kayıt/Enrollment — `enrollment` (V7)
- ogrenciId, grupId, kayitTarihi, durum (`AKTIF`/`AYRILDI`). Mükerrer aktif kayıt → 409. Çıkarma `PATCH /leave`.
- **Uçlar:** `POST/GET /api/enrollments`, filtreler, `/{id}/leave`. Yetki: 3 rol; TEACHER 403.
...
### 7.10 Hakediş — `payout` (V11)
- SAATLIK (oturum × ücret) / CIRO_ORANI (KDV hariç ciro × oran). Uçlar: `/hesapla`, `/onizle`, GET, `/{id}/ode`. ⚠️ SADECE ADMIN.
...
- **Tenant** (`id UUID PK`, `ad`, `status` AKTIF/ASKIDA, `created_at`). Seed: `1111…` Lina, `2222…` Anka.
...
**İş modülleri (liste/form/detay + rol gating):** Öğrenci · Tanımlar · Gruplar/Kayıt · Program/Yoklama · Finans · Hakediş · Stok/Satış · Raporlar · Kullanıcı Yönetimi · Profil. **Dashboard (Genel Bakış)** ✅ YENİ: giriş sonrası role göre dolu panel — `.stat` kartları + recharts trend grafiği (6 ay) + son hareketler + bugünkü dersler; `GET /api/dashboard`'a bağlı, güvenlik backend'de. İlk-şifre kilidi AppShell layout seviyesinde (bypass imkânsız).
...
**✅ Logo (YENİ):** Artademi logosu (gitar+fırça "A" amblemi + "artademi" yazısı). 3 şeffaf varyant `web/src/assets/`: `artademi-logo-full.png` (login/Keycloak teması), `artademi-amblem.png` (sidebar — yatay amblem + Fraunces beyaz "artademi"; renkli yazı koyu zeminde okunmadığından kullanılmaz), `artademi-favicon.png` (`public/`, sekme). AppShell + PlatformShell + ilk-parola ekranı + favicon.
...
> ⚠️ **NOT:** `ARTADEMI_HANDOFF.md` git repo'sunda DEĞİL — sohbet/proje dosyası olarak yaşıyor. Claude Code diskte göremez; güncelleme gerekirse içeriği ona paylaş ya da sohbette güncelle. (İstenirse repoya eklenebilir; içinde test parolaları olduğundan private repo şart.)
...
### 13.1 Platform fazı ✅ TAMAMLANDI
- Tenant CRUD + ASKIDA login engeli + admin provisioning + web konsolu **bitti, push'landı** (commit'ler §11).
- İzolasyon 2 tenant ile kanıtlandı. SUPER_ADMIN tanımı: platform sahibi (Artademi), iş modüllerine fail-closed, yalnız `/api/platform/**`.

### 13.2 İleriye Dönük Altyapı (subscription / SaaS satış) — KALAN BÜYÜK FAZ
> Hedef: ürün online aylık abonelikle satılır. Anka satın alır → login → mail ile gönderilen kullanıcı adı/parola ile girer. İleride otomatik kredi kartı.
- **Tenant subscription** ✅ TAMAMLANDI (V14, commit 17b99e0): plan/dönem/ödeme + grace(14g)/otomatik ASKIDA + DENEME trial. KALAN: gerçek ödeme entegrasyonu + mail bildirimi + grace banner (frontend).
- **Ödeme entegrasyonu (SIRADAKİ BÜYÜK İŞ):** iyzico/PayTR → `paymentStatus`'u gerçek ödemeye bağlar (subscription temeli HAZIR). ⚠️ Sağlayıcı hesabı + sandbox anahtarları + webhook gerekir; "tane tane" curl akışına tam oturmaz (Sercan kendi sandbox'ıyla test eder).
- **Grace uyarı banner:** dashboard'da ADMIN için subscriptionWarning gösteriliyor (✅ kısmen); diğer rollere/sayfalara yaygınlaştırma opsiyonel.
- **Mail bildirimi:** grace başlangıcı/bitişi + ödeme hatırlatma (mail altyapısı kurulunca).
- **Keycloak login sayfası teması** ✅ TAMAMLANDI (commit 3205947): erik-ahududu + logo + tüm login-akışı sayfaları.
- **Şifremi unuttum:** login sayfasına (Keycloak forgot-password + tema).
- **Landing page** + DNS + üretim deploy (Hetzner staging/prod, CI/CD).
- **Frontend "yeni tenant oluştur" formu** ✅ bitti (platform konsolu, §9).

### 13.3 Küçük açık işler
- ✅ TEACHER "kendi gruplarım" (`GET /api/groups/mine`) TAMAMLANDI — tavuk-yumurta çözüldü.
- Finans formlarını RHF+Zod'a hizalama (opsiyonel).

---

## 14. Bilinen Eksikler / Teknik Borç

- ✅ TEACHER "kendi gruplarım" (`GET /api/groups/mine`) çözüldü (CurrentTeacherResolver; oturumsuz yeni grup da görünür).
- **Provisioning'de mail GÖNDERME YOK** — yeni tenant admin'inin username + ilk parolası (Artademi2026!) konsol ekranında gösterilir, super.admin elle iletir. Mail altyapısı ayrı iş.
- **Subscription** ✅ var (V14); ama **gerçek ödeme entegrasyonu YOK** (paymentStatus elle/PATCH ile set ediliyor) ve **mail bildirimi YOK** (grace uyarısı sadece `/api/me` bayrağı; frontend banner da henüz yok).
- `user` modülü: servis-katmanı validasyonları `error.fields` doldurmaz (yalnız `message`); kullanıcı listesinde PageMeta yok.
- Finans inline formları RHF+Zod yerine `useState` (kabul edilmiş istisna).
- Demo modülü (V2 `demo_note`) hâlâ duruyor.
- "Herkes sadece kendi girdiğini düzeltir" ince yetkisi yok. Satış/ödeme iptal/iade yok.
- Keycloak kurulumu elle yapıldı (service account + attribute'lar — §4); script/export'a dökülmedi, yeni ortamda tekrar gerekir.
- `ARTADEMI_HANDOFF.md` repoda değil (§10 notu) — sohbet/proje dosyası olarak tutuluyor.
